...
const SportingDetail = ({ route ,navigation}) => {
  ...
  const [reviewsCount, setReviewsCount] = useState(27);
...
useEffect(() => {
  ...
  return (
     <View style={{width:'59vh',height:'90vh'}}>
    <ScrollView style={{  backgroundColor: '#fff', padding: 10 }}>
      ...
      {/* Hiển thị màu sắc chỉ khi sản phẩm có thuộc tính colors */}
      ...
      <Text style={{ fontSize: 16, marginBottom: 5 }}>• Thương hiệu: {product.brand}</Text>
      <Text style={{ fontSize: 16, marginBottom: 5 }}>• Độ bền: {product.durability}</Text>
